...
/** Modification by Gen Ye:
  *   1. computation error does not terminate MCMC, free memory upon failure:
  *       - modified all function call in evolver_ndf15() except for output and initialization
  *       - modified all class_test in evolver_ndf15()
  *       - fzero_Newton() not touched
*/
...
int evolver_ndf15(
  ...
  class_call_except((*derivs)(t0,y+1,f0+1,parameters_and_workspace_for_derivs,error_message),
                    error_message,
                    error_message,
                    free(buffer);  //free memory upon failure by Gen Ye
        ...
        */
        ...
        for(iter=1;iter<=maxit;iter++){
          ...
          if (jac.use_sparse){
            ...
            class_test_except(funcreturn == _FAILURE_, error_message,
          	     free(buffer);  //free memory upon failure by Gen Ye
                     uninitialize_jacobian(&jac);
                     uninitialize_numjac_workspace(&nj_ws),
        ...
        if (tooslow==_TRUE_){
          ...
          if (Jcurrent==_FALSE_){
            ...
            class_test_except(absh <= hmin, error_message,
          	         free(buffer);  //free memory upon failure by Gen Ye
                     uninitialize_jacobian(&jac);
                     uninitialize_numjac_workspace(&nj_ws),
