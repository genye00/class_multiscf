...
cdef class Class:
    ...
    def get_lnpk_at_z(self, z, nonlinear=False, only_clustering_species = False):
        """
        Returns a grid of log matter power spectrum values at z and log(k)
        Faster because fourier_pk_at_z does not compute interpolation table.

        Parameters
        ----------
        nonlinear : bool
                Whether the returned power spectrum values are linear (default) or non-linear
        nonlinear : bool
        ...
        """
        cdef np.ndarray[DTYPE_t,ndim=1] ln_pk = np.zeros((self.fo.k_size),'float64')
        cdef double *pln_pk = <double*> calloc(self.fo.k_size,sizeof(double))
        cdef np.ndarray[DTYPE_t,ndim=1] ln_k = np.zeros((self.fo.k_size),'float64')
        cdef int index_k

        # consistency checks

        if self.fo.has_pk_matter == False:
            raise CosmoSevereError("You ask classy to return an array of P(k) values at z, but the input parameters sent to CLASS did not require any P(k,z) calculations; add 'mPk' in 'output'")
        ...
        for index_k in xrange(self.fo.k_size):
            ln_k[index_k] = self.fo.ln_k[index_k]

        # get list of P(k) values
        if fourier_pk_at_z(&self.ba,&self.fo,logarithmic,pk_linear,z,self.fo.index_pk_m,pln_pk,NULL)==_FAILURE_:
            raise CosmoSevereError(self.fo.error_message)

        # extract the P(k) values
        for index_k in xrange(self.fo.k_size):
            ln_pk[index_k] = pln_pk[index_k]
    ...
    def get_pk_and_k_and_z(self, nonlinear=True, only_clustering_species = False):
        ...
        cdef int index_k, index_pk
        cdef double z_max_nonlinear, z_max_requested

        # consistency checks

        if self.fo.has_pk_matter == False:
            raise CosmoSevereError("You ask classy to return an array of P(k) values, but the input parameters sent to CLASS did not require any P(k,z) calculations; add 'mPk' in 'output'")
    ...
    def get_current_derived_parameters(self, names):
        ...
        for name in names:
            ...
            # ndescf quantities by Gen Ye
            elif name == 'r_a1':
                value = self.ba.Omega0_ndescf[0]/self.ba.Omega0_m
            elif name == 'm_a1_in_eV':
                value = self.ba.m_ndescf_in_eV[0]
            elif name == 'r_a2':
                value = self.ba.Omega0_ndescf[1]/self.ba.Omega0_m
            elif name == 'm_a2_in_eV':
                value = self.ba.m_ndescf_in_eV[1]
            elif name == 'xi_idr':
