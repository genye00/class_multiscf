                   ...
                   ErrorMsg errmsg)
{
    ...
    char * const target_namestrings[] = {"100*theta_s",
                                         "Omega_dcdmdr",
                                         "omega_dcdmdr",
                                         "Omega_ndescf_stQ",     /* Enable ndescf shooting if equals 1, by Gen Ye */
                                         "Omega_scf",
                                         "Omega_ini_dcdm",
                                         "omega_ini_dcdm",
                                         "sigma8"
                                    ...
                                    ErrorMsg errmsg)
{
    ...
    *needs_shooting = _TRUE_;
    ...
    case Omega_dcdmdr:
    case omega_dcdmdr:
    case Omega_ndescf_stQ: // for ndescf, by Gen Ye
        /* != 1. means no shooting */
        if(target_value != 1.)
                    ...
                    ErrorMsg errmsg)
{
    ...
    /** fixed point search for Omega_ndescf_tot by Gen Ye
      * more reliable in case of Omega_ndescf_tot */
    if(pfzw->target_name[0] == Omega_ndescf_stQ)
    {
        while(*fevals<200)
        {
            if(fabs(fabs(f1)<1e-5))
            {
                *xzero = x1;
                return _SUCCESS_;
            }
        ...
        printf("------->Fixed point search for Omega_ndescf_tot failed after %d iterations<-------\n",*fevals);
    }
                    ...
                    ErrorMsg errmsg)
{
    ...
    for(index_guess = 0; index_guess < pfzw->target_size; index_guess++)
    {
        ...
        {
        case theta_s:
            xguess[index_guess] = 3.54 * pow(pfzw->target_value[index_guess],
                                             2) - 5.455 * pfzw->target_value[index_guess] + 2.548;
                                 ...
                                 ErrorMsg errmsg)
{
    ...
    if(pfzw->required_computation_stage >= cs_thermodynamics)
    {
        ...
        th.hyrec_verbose = 0;
        class_call_except(thermodynamics_init(&pr, &ba, &th), th.error_message, errmsg,
                          background_free(&ba));
    }
    ...
    if(pfzw->required_computation_stage >= cs_perturbations)
    {
        ...
        class_call_except(perturbations_init(&pr, &ba, &th, &pt), pt.error_message, errmsg,
    ...
    if(pfzw->required_computation_stage >= cs_primordial)
    {
        ...
        class_call_except(primordial_init(&pr, &pt, &pm), pm.error_message, errmsg, perturbations_free(&pt);
                          thermodynamics_free(&th); background_free(&ba));
    }
    ...
    if(pfzw->required_computation_stage >= cs_nonlinear)
    {
        ...
        class_call_except(fourier_init(&pr, &ba, &th, &pt, &pm, &fo), fo.error_message, errmsg,
                          primordial_free(&pm); perturbations_free(&pt); thermodynamics_free(&th); background_free(&ba));
    }
    ...
    if(pfzw->required_computation_stage >= cs_transfer)
    {
        ...
        class_call_except(transfer_init(&pr, &ba, &th, &pt, &fo, &tr), tr.error_message, errmsg,
                          fourier_free(&fo); primordial_free(&pm); perturbations_free(&pt); thermodynamics_free(&th);
                          background_free(&ba));
    }
    ...
    if(pfzw->required_computation_stage >= cs_spectra)
    {
        ...
        class_call_except(harmonic_init(&pr, &ba, &pt, &pm, &fo, &tr, &hr), hr.error_message, errmsg,
                          transfer_free(&tr); fourier_free(&fo); primordial_free(&pm); perturbations_free(&pt);
                          thermodynamics_free(&th); background_free(&ba));
    }
    ...
    for(i = 0; i < pfzw->target_size; i++)
    {
        ...
        case omega_dcdmdr:
            ...
            break;
        case Omega_ndescf_stQ: // ndescf shooting, by Gen Ye
            output[i] = ba.Omega0_ndescf_tot - ba.Omega0_ndescf_tot_real;
            //printf("Delta Omega0_ndescf = %g.\n",output[i]);
            break;
        case Omega_scf:
                                  ...
                                  ErrorMsg errmsg)
{
    ...
    if(flag1 == _TRUE_)
    {
        ...
        if((strstr(string1, "Sd") != NULL) || (strstr(string1, "sd") != NULL)
                || (strstr(string1, "SD") != NULL))
        {
            ppt->has_perturbations = _TRUE_;
        ...
        class_test(flag1 == _FALSE_,
                   errmsg, "The options for output are {'tCl','pCl','lCl','nCl','dCl','sCl','mPk','mTk','dTk','vTk','Sd'}, you entered '%s'",
                   string1);
    }
    ...
    if(ppt->has_cl_cmb_temperature == _TRUE_)
    {
        ...
        if(flag1 == _TRUE_)
        {
            ...
            class_call(parser_check_options(string1, options_temp_contributions, 10, &flag1),
                       ...
                       errmsg);
            class_test(flag1 == _FALSE_,
                       errmsg, "The options for 'temperature_contributions' are {'tsw','eisw','lisw','dop','pol'}, you entered '%s'",
                       string1);
            class_test((ppt->switch_sw == 0) && (ppt->switch_eisw == 0) && (ppt->switch_lisw == 0)
                       && (ppt->switch_dop == 0) && (ppt->switch_pol == 0),
                       errmsg,
                       "You specified 'temperature_contributions' as '%s'. It has to contain some of {'tsw','eisw','lisw','dop','pol'}.",
                       string1);
    ...
    if(ppt->has_cl_number_count == _TRUE_)
    {
        ...
        if(flag1 == _TRUE_)
        {
            ...
            class_call(parser_check_options(string1, options_number_count, 8, &flag1),
                       ...
                       errmsg);
            class_test(flag1 == _FALSE_,
                       errmsg, "The options for 'number_count_contributions' are {'density','rsd','lensing','gr'}, you entered '%s'",
                       string1);
            class_test((ppt->has_nc_density == _FALSE_) && (ppt->has_nc_rsd == _FALSE_)
                       && (ppt->has_nc_lens == _FALSE_) && (ppt->has_nc_gr == _FALSE_),
                       errmsg,
                       "You specified 'number_count_contributions' as '%s'. It has to contain some of {'density','rsd','lensing','gr'}.",
                       string1);
        }
        else
        {
    ...
    {
        /* Read */
        class_read_flag_or_deprecated("extra_metric_transfer_functions", "extra metric transfer functions",
                                      ppt->has_metricpotential_transfers);
    }
    ...
    if(ppt->has_perturbations == _TRUE_)
    {
        ...
        /* Read */
        class_read_flag_or_deprecated("perturbed_recombination", "perturbed recombination",
    ...
    if(flag1 == _TRUE_)
    {
        ...
        else if((strstr(string1, "synchronous") != NULL) || (strstr(string1, "sync") != NULL)
    ...
    if((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_))
    {
        ...
        /* Read */
        class_read_flag_or_deprecated("nbody_gauge_transfer_functions", "Nbody gauge transfer functions",
                                      ppt->has_Nbody_gauge_transfers);
    ...
    if(flag1 == _TRUE_)
    {
        if((strstr(string1, "RECFAST") != NULL) || (strstr(string1, "recfast") != NULL)
                || (strstr(string1, "Recfast") != NULL))
        {
            pth->recombination = recfast;
        }
        else if((strstr(string1, "HYREC") != NULL) || (strstr(string1, "hyrec") != NULL)
    ...
    if(pth->recombination == recfast)
    {
        ...
        if(flag1 == _TRUE_)
        {
            if((strstr(string1, "Tmat") != NULL) || (strstr(string1, "tmat") != NULL)
                    || (strstr(string1, "TMAT") != NULL))
            {
                pth->recfast_photoion_mode = recfast_photoion_Tmat;
            }
            else if((strstr(string1, "Trad") != NULL) || (strstr(string1, "trad") != NULL)
                    || (strstr(string1, "TRAD") != NULL))
            {
            ...
            {
                class_stop(errmsg,
                           "You specified 'recfast_photoion_dependence' as '%s'. It has to be one of {'Tmat','Trad'}.",
                           string1);
            }
        }
    }
    ...
    if(flag1 == _TRUE_)
    {
        ...
        {
            class_stop(errmsg,
                       "You specified 'reio_parametrization' as '%s'. It has to be one of {'reio_none','reio_camb','reio_bins_tanh','reio_half_tanh','reio_many_tanh','reio_inter'}.",
                       string1);
        }
    }
    ...
    default:
        class_stop(pth->error_message, "pth->recombination=%d different from all known cases",
                   pth->recombination);
        break;
    }
    ...
    /** 9) Damping scale */
    /* Read */
    class_read_flag_or_deprecated("compute_damping_scale", "compute damping scale",
                                  pth->compute_damping_scale);
                                  ...
                                  ErrorMsg errmsg)
{
    ...
    double Omega_r, Omega_m, rho_eq; //for use in the ADE and ndescf case by Gen Ye
    int ndescf_sorted; // ndescf param


    sigma_B = 2.*pow(_PI_, 5.) * pow(_k_B_, 4.) / 15. / pow(_h_P_, 3.) / pow(_c_,
              2); // [W/(m^2 K^4) = Kg/(K^4 s^3)]
    ...
    {
        pba->Omega0_g = (4.*sigma_B / _c_ * pow(pba->T_cmb,
                                                4.)) / (3.*_c_ * _c_ * 1.e10 * pba->h * pba->h / _Mpc_over_m_ / _Mpc_over_m_ / 8. / _PI_ / _G_);
    }
    else
    {
        if(flag1 == _TRUE_)
        {
            pba->Omega0_g = (4.*sigma_B / _c_ * pow(param1,
                                                    4.)) / (3.*_c_ * _c_ * 1.e10 * pba->h * pba->h / _Mpc_over_m_ / _Mpc_over_m_ / 8. / _PI_ / _G_);
            pba->T_cmb = param1;
        }
        if(flag2 == _TRUE_)
        {
            pba->Omega0_g = param2;
            pba->T_cmb = pow(pba->Omega0_g * (3.*_c_ * _c_ * 1.e10 * pba->h * pba->h / _Mpc_over_m_ /
                                              _Mpc_over_m_ / 8. / _PI_ / _G_) / (4.*sigma_B / _c_), 0.25);
        }
        if(flag3 == _TRUE_)
        {
    ...
    if((ppt->gauge == synchronous)
            && (pba->Omega0_cdm == 0)) pba->Omega0_cdm = ppr->Omega0_cdm_min_synchronous;
    ...
    if(N_ncdm > 0)
    {
        ...
        if(fileentries > 0)
        {
            ...
            /* Read */
            class_call(parser_read_list_of_strings(pfc, "ncdm_psd_filenames", &entries_read,
                                                   &(pba->ncdm_psd_files), &flag1, errmsg),
                       errmsg,
                       errmsg);
            /* Test */
        ...
        /* Read */
        parser_read_list_of_doubles(pfc, "ncdm_psd_parameters", &entries_read, &(pba->ncdm_psd_parameters),
                                    &flag1, errmsg);
        ...
        /* Read */
        // comment out duplicated allocation to avoid memory leak by Gen Ye
//        class_read_list_of_integers_or_default("Quadrature strategy", pba->ncdm_quadrature_strategy, 0,
//                                               N_ncdm); //Deprecated parameter, still read to keep compatibility
        class_read_list_of_integers_or_default("ncdm_quadrature_strategy", pba->ncdm_quadrature_strategy, 0,
                                               N_ncdm);
...
//        class_read_list_of_integers_or_default("Number of momentum bins", pba->ncdm_input_q_size, 150,
//                                               N_ncdm); //Deprecated parameter, still read to keep compatibility
    ...
    if(pba->Omega0_dcdmdr > 0)
    {
        ...
        /* Read */
        class_call(parser_read_double(pfc, "Gamma_dcdm", &param1, &flag1,
                                      errmsg),                      // [km/(s Mpc)]
                   errmsg,
                   errmsg);
        class_call(parser_read_double(pfc, "tau_dcdm", &param2, &flag2,
                                      errmsg),                        // [s]
                   errmsg,
                   errmsg);
        /* Test */
        ...
        if(flag1 == _TRUE_)
        {
            pba->Gamma_dcdm = param1 * (1.e3 /
                                        _c_);                                                      // [Mpc]
            pba->tau_dcdm = 1 / (param1 * 1.02e-3) * (1e9 * 365 * 24 *
                            3600);                                         // [s]
        }//TODO :: fix these factors to be proper (i.e. no 1.02e-3)
        if(flag2 == _TRUE_)
        {
            pba->Gamma_dcdm = 1 / (param2 / (1e9 * 365 * 24 * 3600)) / 1.02e-3 * (1.e3 /
                              _c_);                          // [Mpc]
    ...
    if(flag1 == _TRUE_)
    {
        pba->T_idr = pow(param1 / stat_f_idr * (7. / 8.) / pow(11. / 4., (4. / 3.)),
                         (1. / 4.)) * pba->T_cmb;
        if(input_verbose > 1)
    ...
    {
        pba->T_idr = pow(param2 / stat_f_idr * (7. / 8.) / pow(11. / 4., (4. / 3.)),
                         (1. / 4.)) * pba->T_cmb;
        if(input_verbose > 2)
    ...
    if(flag3 == _TRUE_)
    {
        class_test((param3 < 0.) || (param3 > 1.),
                   errmsg,
                   "The fraction of interacting DM with DR must be between 0 and 1, you asked for f_idm_dr=%e",
                   param3);
    ...
    if(pba->Omega0_idm_dr > 0.)
    {
        ...
        else
        {
            ...
            /** 7.2.e.4) idr_nature */ class_call(parser_read_string(pfc, "idr_nature", &string1, &flag1,
                                                  errmsg),
                                                  errmsg,
                                                  errmsg);
            ...
            if(flag1 == _TRUE_)
            {
                if((strstr(string1, "free_streaming") != NULL) || (strstr(string1, "Free_Streaming") != NULL)
                        || (strstr(string1, "Free_streaming") != NULL) || (strstr(string1, "FREE_STREAMING") != NULL))
                {
...
//    {
        ...
        class_call(parser_read_list_of_doubles(pfc, "alpha_idm_dr", &entries_read, &(ppt->alpha_idm_dr),
                                               &flag1, errmsg),
                   errmsg,
                   errmsg);
        ...
        if(flag1 == _FALSE_)
        {
            class_call(parser_read_list_of_doubles(pfc, "alpha_dark", &entries_read, &(ppt->alpha_idm_dr),
                                                   &flag1, errmsg),
                       errmsg,
                       errmsg);
        }
        ...
        if(ppt->has_perturbations)
        {
            if(flag1 == _TRUE_)
            {
                if(entries_read != (ppr->l_max_idr - 1))
                {
                    ...
                    for(n = entries_read; n < (ppr->l_max_idr - 1);
                            n++) ppt->alpha_idm_dr[n] = ppt->alpha_idm_dr[entries_read - 1];
                }
            }
            else
            {
        ...
        class_call(parser_read_list_of_doubles(pfc, "beta_idr", &entries_read, &(ppt->beta_idr), &flag1,
                                               errmsg),
                   errmsg,
                   errmsg);
        ...
        if(flag1 == _FALSE_)
        {
            class_call(parser_read_list_of_doubles(pfc, "beta_dark", &entries_read, &(ppt->beta_idr), &flag1,
                                                   errmsg),
                       errmsg,
                       errmsg);
        }
        ...
        if(ppt->has_perturbations)
        {
            if(flag1 == _TRUE_)
            {
                if(entries_read != (ppr->l_max_idr - 1))
                {
                    ...
                    for(n = entries_read; n < (ppr->l_max_idr - 1);
                            n++) ppt->beta_idr[n] = ppt->beta_idr[entries_read - 1];
                }
            }
            else
            {
    ...
    /* Add several scalar fields, by Gen Ye */
    /** 8) Non dark energy Scalar field */

    /* adaptive step background integration, useful in the ndescf case */
    class_call(parser_read_string(pfc, "adaptive_step_bk_sampling", &string1, &flag1, errmsg),
           errmsg,
           errmsg);
    if(flag1 == _TRUE_)
    {
        ...
        {
            pba->adaptive_step_bk_sampling = _TRUE_;
        }
    }

    /* Scalar field not used to fill, usually oscillating fields that do not behave as dark energy */
    /* Read */
    // TODO: read in omega_ndescf[i] for each field instead, then shoot for ndescf_tuning_parameters
    class_call(parser_read_int(pfc, "N_ndescf", &(pba->N_ndescf), &flag1, errmsg),
                   errmsg,
                   errmsg);

    /* If we actually have non dark energy scalar field */
    if(pba->N_ndescf>0){
        /* Read in the number of parameters for one field.
         * All ndescf are assumed to have the samenumber of parameters.
         * The last two entries should always be the initial state phi_n_init and phi_n_prime_init. */
        class_call(parser_read_int(pfc, "ndescf_parameters_size", &(pba->ndescf_parameters_size), &flag1, errmsg),
                   errmsg,
                   errmsg);
        /* We does not try to deduce number of parameters for each field. Should be explicitly specified. */
        class_test(flag1 == _FALSE_,
                       errmsg,
                       "Please specify the number of parameters for each non dark energy scalar fields.");
        /* Last 2 entries always for initial state, and the last third for potential type. */
        class_test(pba->ndescf_parameters_size<3*pba->N_ndescf,
                       errmsg,
                       "Too few parameters for ndescf. You must at least specify phi ,its derivative phi' and potential type for each field.");

        /* Read in the parameters for scalar fields */
        class_call(parser_read_list_of_doubles(pfc, "ndescf_parameters", &entries_read, &(pba->ndescf_parameters),
                                                   &flag1, errmsg),
                       errmsg,
                       errmsg);
        /* Test */
        class_test(flag1 == _FALSE_,
                       errmsg,
                       "You must specify ndescf_parameters in the input .ini file.");
        class_test(pba->N_ndescf*pba->ndescf_parameters_size!=entries_read,
                       errmsg,
                       "Expect %d entries in ndescf_parameters, but got %d.",pba->N_ndescf*pba->ndescf_parameters_size,entries_read);

        /* Read in fld approximation energy scale ndescf_m_fld in unit of H0.
         * Fluid approximation is used for m/H >= ndescf_fld_threshold */
        class_call(parser_read_list_of_doubles(pfc, "ndescf_m_fld", &entries_read, &(pba->ndescf_m_fld), &flag1,
                                               errmsg),
                   errmsg,
                   errmsg);
        class_test(flag1 == _FALSE_,
                   errmsg,
                   "You must specify fluid approximation scale. Set to 0 if you want to use Klein-Gorden eq. throughout.");
        class_test(entries_read != pba->N_ndescf,
                   errmsg,
                   "Expect %d entries in ndescf_m_fld, but got %d.",pba->N_ndescf,entries_read);

        /* Read in fld approx threshold */
        class_call(parser_read_double(pfc, "ndescf_fld_threshold", &(pba->ndescf_fld_threshold), &flag1, errmsg),
                   errmsg,
                   errmsg);

        /* Allocate relevant arrays */
        class_alloc(pba->ndescf_w_fld, sizeof(double)*pba->N_ndescf, pba->error_message);
        class_alloc(pba->ndescf_tau_fld, sizeof(double)*pba->N_ndescf, pba->error_message);
        class_alloc(pba->ndescf_loga_fld, sizeof(double)*pba->N_ndescf, pba->error_message);
        class_alloc(pba->ndescf_tau_avgfld, sizeof(double)*pba->N_ndescf, pba->error_message);
        class_alloc(pba->ndescf_sorted_index, sizeof(int)*pba->N_ndescf, pba->error_message);

        /* Loop over all fields:
         *   1. Assign effective EoS params for fld approx, this is determined by potential type
         *   2. Deduce scale factor, in fact a/a_0, at which fld approx begins
         *   3. Some initialization */
        Omega_r = pba->Omega0_g * (1. + 3.044 * 7. / 8.*pow(4. / 11., 4. / 3.));
        Omega_m = pba->Omega0_b + pba->Omega0_cdm + pba->Omega0_idm_dr;
        rho_eq = 2.45*Omega_m*Omega_m/pow(Omega_r,1.5); // in unit of H0^2 M_p^2
        for(n=0;n<pba->N_ndescf;n++)
        {
            /* Assign EoS params, currently only axion potential is implemented */
            if(pba->ndescf_parameters[pba->ndescf_parameters_size*(n+1)-3]<0)
            {
                pba->ndescf_w_fld[n] = 0.;
                // m given in unit of H0
                pba->ndescf_parameters[pba->ndescf_parameters_size*n+1] *= pba->H0;
            }

            /* Estimate time when fld approx should be turned on */
            // not exact, but should not affect cosmology if ndescf_fld_threshold is sufficiently large
            pba->ndescf_loga_fld[n] = pba->ndescf_m_fld[n] / pba->ndescf_fld_threshold;
            if(pba->ndescf_loga_fld[n] > rho_eq)
            {
                // assuming radiation domination
                pba->ndescf_loga_fld[n] = 0.25 * log(3*Omega_r/pba->ndescf_loga_fld[n]/pba->ndescf_loga_fld[n]);
            }
            else // assuming matter domination
            {
                pba->ndescf_loga_fld[n] = 0.333 * log(3*Omega_m/pba->ndescf_loga_fld[n]/pba->ndescf_loga_fld[n]);
            }
            //printf("rho_eq = %g, z_fld=%g\n", rho_eq,1/exp(pba->ndescf_loga_fld[n])+1);

            pba->ndescf_m_fld[n] *= pba->H0;

            /* initialize sorted indices */
            pba->ndescf_sorted_index[n] = n;
            /* initialize conformal time of fld approx to some safely big value, in unit of Mpc */
            pba->ndescf_tau_fld[n] = 1e10;
            pba->ndescf_tau_avgfld[n] = 1e10;
        }

        /* sort ndescf_loga_fld from earliest to latest */
        ndescf_sorted = pba->N_ndescf-1; // sort if more than one fields
        while(ndescf_sorted>0)
        {
            ndescf_sorted = 0;
            for(n=1;n<pba->N_ndescf;n++)
            {
                if(pba->ndescf_loga_fld[pba->ndescf_sorted_index[n-1]]>pba->ndescf_loga_fld[pba->ndescf_sorted_index[n]])
                {
                    pba->ndescf_sorted_index[n-1], pba->ndescf_sorted_index[n] = pba->ndescf_sorted_index[n], pba->ndescf_sorted_index[n-1];
                    ndescf_sorted+=1;
                }
            }
        }

        class_alloc(pba->ndescf_use_kg, sizeof(short)*pba->N_ndescf, pba->error_message);

        /* TODO: Omega0_ndescf as shooting target */
        class_alloc(pba->Omega0_ndescf, sizeof(double)*pba->N_ndescf, pba->error_message);
        class_alloc(pba->m_ndescf_in_eV, sizeof(double)*pba->N_ndescf, pba->error_message);

        /* Read in Omega_ndescf_tot. Currently this should be produced by shooting */
        class_read_double("Omega_ndescf_tot", pba->Omega0_ndescf_tot);
    }
    /* end of midification by Gen Ye */
    ...
    Omega_tot += pba->Omega0_ndescf_tot; // ndescf contribution, by Gen Ye
    /* Step 1 */
    if(flag1 == _TRUE_)
    {
    ...
    if(flag2 == _TRUE_)
    {
        ...
        /* Handle ADE (arXiv:1905.12618) case here, in which Omega0_fld is deduced based on parameters, by Gen Ye */
        // ADE is not intended for filling the budget equation
        class_call(parser_read_string(pfc, "fluid_equation_of_state", &string1, &flag1, errmsg),
                   errmsg,
                   errmsg);
        if((flag1==_TRUE_)&&((strstr(string1, "ADE") != NULL) || (strstr(string1, "ade") != NULL)))
        {
            class_stop(pba->error_message,
                       "ADE not implemented yet.");
            /** 9.a.2.4) Equation of state of the fluid in 'ADE' case */
            pba->fluid_equation_of_state = ADE;
            class_read_double("wf_ADE", pba->wf_ADE);
            class_read_double("a_ADE", pba->a_ADE);
            class_read_double("f_ADE", pba->f_ADE);
            class_read_double("cs2_ADE", pba->cs2_ADE);
            class_read_double("cvis2_ADE", pba->cvis2_ADE);
            // not exact but good enough
            Omega_r = pba->Omega0_g * (1. + 3.044 * 7. / 8.*pow(4. / 11., 4. / 3.));
            Omega_m = pba->Omega0_b + pba->Omega0_cdm + pba->Omega0_idm_dr;
            pba->Omega0_fld = pba->f_ADE * (Omega_m * pow(pba->a_ADE, 3*pba->wf_ADE)+Omega_r*pow(pba->a_ADE,3*pba->wf_ADE-1));
        }
        /* end of modification by Gen Ye */
    ...
    if(pba->Omega0_fld != 0.)
    {
        ...
        if(flag1 == _TRUE_)
        {
            ...
            else if((strstr(string1, "PDF") != NULL) || (strstr(string1, "pdf") != NULL))
            {
                // dealt with above when Omega_fld is read, by Gen Ye
            }
            else
            {
    ...
    /* Force scf calculation when Omega_scf=0, useful for EDE negligible today, by Gen Ye*/
    class_call(parser_read_string(pfc, "force_ede", &string1, &flag1, errmsg), errmsg, errmsg);
    if(flag1 == _TRUE_)
    {
        if(string_begins_with(string1, 'y') || string_begins_with(string1, 'Y'))
        {
            pba->force_ede = _TRUE_;
        }
    }

    /** 9.b) If Omega scalar field (SCF) is different from 0 */
    if(pba->Omega0_scf != 0. || pba->force_ede)
    /*end of modification by Gen Ye*/
    {
        ...
        if(flag1 == _TRUE_)
        {
            if(string_begins_with(string1, 'y') || string_begins_with(string1, 'Y'))
            {
                class_test(pba->force_ede == _TRUE_,
                           errmsg,
                           "EDE is not compatible with attractor initial conditions");
        ...
        {
            printf("'scf_lambda' = %e < 3 won't be tracking (for exp quint) unless overwritten by tuning function.",
                   scf_lambda);
        }
    }
                                    ...
                                    ErrorMsg errmsg)
{
    ...
    class_test(pin->DM_annihilation_efficiency > 1.e-4,
               ...
               pin->DM_annihilation_efficiency);
    ...
    class_test((pin->DM_annihilation_cross_section != 0 || pin->DM_annihilation_mass != 0
                || pin->DM_annihilation_fraction != 0) && pin->DM_annihilation_efficiency != 0,
               errmsg,
    ...
    if((pin->DM_annihilation_efficiency > 0) && (pth->reio_parametrization == reio_none)
    ...
    {
        pin->DM_annihilation_efficiency = pin->DM_annihilation_cross_section * 1.e-6 /
                                          (pin->DM_annihilation_mass * _eV_ * 1.e9);
    }
    ...
    if(flag1 == _TRUE_)
    {
        ...
        {
            class_stop(errmsg,
                       "You specified 'PBH_accretion_recipe' as '%s'. It has to be one of {'spherical_accretion','disk_accretion'}.",
                       string1);
        }
    }
    ...
    if(pin->PBH_accretion_recipe == disk_accretion)
    {
        ...
        /* Test */
        class_test(pin->PBH_accretion_ADAF_delta != 1e-3 && pin->PBH_accretion_ADAF_delta != 0.5
                   && pin->PBH_accretion_ADAF_delta != 0.1,
                   errmsg,
    ...
    if(flag1 == _TRUE_)
    {
        ...
        {
            class_stop(errmsg,
                       "You specified 'chi_type' as '%s'. It has to be one of {'CK_2004','PF_2005','Galli_2013_file','Galli_2013_analytic','heat','from_x_file','from_z_file'}.",
                       string1);
        }
    }
                                    ...
                                    ErrorMsg errmsg)
{
    ...
    if(flag1 == _TRUE_)
    {
        ...
        else if((strstr(string1, "hmcode") != NULL) || (strstr(string1, "HMCODE") != NULL)
                                     ...
                                     ErrorMsg errmsg)
{
    ...
    if(flag1 == _TRUE_)
    {
        ...
        {
            class_stop(errmsg,
                       "You specified 'P_k_ini_type' as '%s'. It has to be one of {'analytic_Pk','inflation_V','inflation_V_end','two_scales','external_Pk'}.",
                       string1);
        }
    }
    ...
    if(ppm->primordial_spec_type == analytic_Pk)
    {
        ...
        if(ppt->has_tensors == _TRUE_)
        {
            ...
            else
            {
                ...
                {
                    ppm->n_t = -ppm->r / 8.*(2. - ppm->r / 8. -
                                             ppm->n_s); // enforce single slow-roll self-consistency condition (order 2 in slow-roll)
                }
                ...
                {
                    ppm->alpha_t = ppm->r / 8.*(ppm->r / 8. + ppm->n_s -
                                                1.); // enforce single slow-roll self-consistency condition (order 2 in slow-roll)
                }
            }
        }
    }
    ...
    else if((ppm->primordial_spec_type == inflation_V) || (ppm->primordial_spec_type == inflation_H))
    {
        ...
        if(ppm->primordial_spec_type == inflation_V)
        {
            ...
            /* Read */
            class_call(parser_read_string(pfc, "potential", &string1, &flag1,
                                          errmsg), // only polynomial coded so far:
    ...
    else if(ppm->primordial_spec_type == inflation_V_end)
    {
        ...
        if(flag1 == _TRUE_)
        {
            ...
            {
                class_stop(errmsg,
                           "You specified 'full_potential' as '%s'. It has to be one of {'polynomial','higgs_inflation'}.",
                           string1);
            }
        }
        ...
        if(flag1 == _TRUE_)
        {
            ...
            {
                class_stop(errmsg,
                           "You specified 'inflation_behavior' as '%s'. It has to be one of {'numerical','analytical'}.",
                           string1);
            }
        }
    }
    ...
    else if(ppm->primordial_spec_type == two_scales)
    {
        ...
        if(ppt->has_scalars == _TRUE_)
        {
                    ...
                    || (ppt->has_niv == _TRUE_))
            {
                ...
                /* Complete set of parameters */
                if((flag1 == _TRUE_)
                        && (strstr(string1, "axion") != NULL))     // Axion case, only one iso parameter: piir1
                {
                    n_iso = 1.;
                    n_cor = 0.;
                    c_cor = 0.;
                        ...
                        && (strstr(string1, "anticurvaton") != NULL))       // Curvaton case, only one iso parameter: piir1
                {
                    n_iso = ppm->n_s;
                    n_cor = 0.;
                    c_cor = 1.;
                        ...
                        && (strstr(string1, "curvaton") != NULL))       // inverted-correlation-curvaton case, only one iso parameter: piir1
                {
                    n_iso = ppm->n_s;
                    n_cor = 0.;
                    c_cor = -1.;
                ...
                else           // general case, but if pii2 or pri2=0, the code interprets it as a request for n_iso=n_ad or n_cor=0 respectively
                {
                    ...
                    class_test(fabs(pri1) / sqrt(pii1 * prr1)*exp(n_cor * log(k2 / k1)) > 1, errmsg,
                                  ...
                                  ErrorMsg errmsg)
{
    ...
    if((ppt->has_cl_number_count == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_))
    {
        ...
        if(flag1 == _TRUE_)
        {
            ...
            {
                class_stop(errmsg,
                           "You specified 'selection' as '%s'. It has to be one of {'gaussian','tophat','dirac'}.", string1);
            }
        }
        ...
        if((flag1 == _TRUE_) && (int1 > 0))
        {
            ...
            for(i = 0; i < int1; i++)
            {
                ...
                class_test((pointer1[i] < 0.) || (pointer1[i] > 1000.), errmsg,
                           "input of selection functions: you asked for a mean redshift equal to %e, is this a mistake?",
                           pointer1[i]);
                /* Complete set of parameters */
            ...
            for(i = 1; i < int1; i++)      // first set all widths to default; correct eventually later
            {
                ...
                class_test(ppt->selection_mean[i] <= ppt->selection_mean[i - 1],
                           errmsg,
                           "input of selection functions: the list of mean redshifts must be passed in growing order; you entered %e before %e.",
            ...
            /* Read */
            class_call(parser_read_list_of_doubles(pfc, "selection_magnification_bias", &int1, &pointer1,
                                                   &flag1, errmsg),
                       errmsg,
                       errmsg);
            /* Complete set of parameters */
        ...
        else
        {
            ...
            if((ppt->has_cl_number_count == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_))
            {
                for(bin = 0; bin < ppt->selection_num; bin++)
                {
                    ...
                    {
                        z_max = ppt->selection_mean[bin] + (1. + ppr->selection_cut_at_sigma * ppr->selection_tophat_edge) *
                                  ...
                                  ErrorMsg errmsg)
{
    ...
    {
        if((ppt->has_scalars == _TRUE_) && ((ppt->has_cl_cmb_temperature == _TRUE_)
                                            || (ppt->has_cl_cmb_polarization == _TRUE_)) && (ppt->has_cl_cmb_lensing_potential == _TRUE_))
        {
        ...
        {
            class_stop(errmsg,
                       "you asked for lensed CMB Cls, but this requires a minimal number of options: 'modes' should include 's', 'output' should include 'tCl' and/or 'pCl', and also, importantly, 'lCl', the CMB lensing potential spectrum.");
        }
    }
                                      ...
                                      ErrorMsg errmsg)
{
    ...
    if(flag1 == _TRUE_)
    {
        ...
        {
            class_stop(errmsg,
                       "You specified 'branching_approx' as '%s'. It has to be one of {'sharp_sharp','sharp_soft','soft_soft','soft_soft_cons','exact'}.",
                       string1);
        }
    }
    ...
    if(psd->sd_branching_approx == bra_exact)
    {
        ...
        if(flag1 == _TRUE_)
        {
            ...
            psd->sd_detector_bin_number = ((int)ceil((psd->sd_detector_nu_max - psd->sd_detector_nu_min) /
                                           param1));
            psd->has_user_defined_detector = _TRUE_;
        }
        if(flag2 == _TRUE_)
    ...
    if(psd->has_SZ_effect == _TRUE_)
    {
        ...
        if(flag1 == _TRUE_)
        {
            ...
            {
                class_stop(errmsg,
                           "You specified 'sd_reio_type' as '%s'. It has to be one of {'Nozawa_2005','Chluba_2012'}.",
                           string1);
            }
        }
    }
                                     ...
                                     ErrorMsg errmsg)
{
    ...
    class_call(parser_read_string(pfc, "l_switch_limber_for_cl_density_over_z", &string1, &flag1,
                                  errmsg),
               errmsg,
               errmsg);
    class_test(flag1 == _TRUE_,
                                 ...
                                 ErrorMsg errmsg)
{
    ...
    if(flag1 == _TRUE_)
    {
        class_test(strlen(string1) > _FILENAMESIZE_ - 32, errmsg,
                   "Root directory name is too long. Please install in other directory, or increase _FILENAMESIZE_ in common.h");
    ...
    if(flag1 == _TRUE_)
    {
        ...
        {
            class_stop(errmsg, "You specified 'format' as '%s'. It has to be one of {'class','camb'}.",
                       string1);
        }
    }
    ...
    /* Read */
    class_read_flag_or_deprecated("write_thermodynamics", "write thermodynamics",
                                  pop->write_thermodynamics);
    ...
    if(flag1 == _TRUE_)
    {
        ...
        qsort(ppt->k_output_values, ppt->k_output_values_num, sizeof(double),
              compare_doubles);      // Sort the k_array using qsort
    ...
    /* Read */
    class_read_flag_or_deprecated("write_exotic_injection", "write exotic injection",
                                  pop->write_exotic_injection);
                         ...
                         struct output *pop)
{
    ...
    /* ** ADDITIONAL SPECIES ** */
    /* add non dark energy scalar field, by Gen Ye */
    /** 8) Non dark energy scalar fields */
    pba->N_ndescf = 0;
    pba->Omega0_ndescf = NULL;
    pba->Omega0_ndescf_tot = 0.;
    pba->Omega0_ndescf_tot_real = 0.;
    pba->ndescf_parameters = NULL;
    pba->ndescf_parameters_size = 0;
    pba->ndescf_loga_fld = NULL;
    pba->ndescf_tau_fld = NULL;
    pba->ndescf_tau_avgfld = NULL;
    pba->ndescf_w_fld = NULL;
    pba->ndescf_m_fld = NULL;
    pba->ndescf_fld_threshold = 2e2;
    pba->ndescf_sorted_index = NULL;
    pba->ndescf_use_kg = NULL;
    pba->adaptive_step_bk_sampling = _FALSE_;
    /* end of modification by Gen Ye */
    ...
    pba->Omega0_lambda = 1. - pba->Omega0_k - pba->Omega0_g - pba->Omega0_ur - pba->Omega0_b -
                         pba->Omega0_cdm - pba->Omega0_ncdm_tot - pba->Omega0_dcdmdr + pba->Omega0_idr + pba->Omega0_idm_dr;
    /** 9.a) Omega fluid */
    /** 9.a.1) PPF approximation */
    pba->use_ppf = _TRUE_;
    ...
    pba->force_ede = _FALSE_;  // do not force scf evolution by default, by Gen Ye
